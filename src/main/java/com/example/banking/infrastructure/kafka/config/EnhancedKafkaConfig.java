 ...
 * 
 * 1. kafkaListenerContainerFactory: Para comandos (ya existente)
 * 2. materializerKafkaListenerContainerFactory: Para materialización de eventos
 * 3. commandReplyKafkaListenerContainerFactory: Para respuestas de comandos
 */
@Slf4j
@Configuration
@EnableKafka
public class EnhancedKafkaConfig {
    ...
    public ConsumerFactory<String, String> materializerConsumerFactory() {
        ...
        configProps.put(ConsumerConfig.GROUP_ID_CONFIG, "${kafka.events.materializer.group-id}");
        configProps.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
        configProps.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
        configProps.put(ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, "earliest");
        configProps.put(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, false);
        configProps.put(ConsumerConfig.MAX_POLL_RECORDS_CONFIG, 500); // Batch processing
        configProps.put(ConsumerConfig.FETCH_MIN_BYTES_CONFIG, 1024); // Esperar más datos
        configProps.put(ConsumerConfig.FETCH_MAX_WAIT_MS_CONFIG, 500);
        
        log.info("✅ Configurando MaterializerConsumer con group-id: ${kafka.events.materializer.group-id}");
    ...
    public ConcurrentKafkaListenerContainerFactory<String, String> materializerKafkaListenerContainerFactory(
        ...
        factory.setConcurrency(5); // Alta concurrencia para materialización
    ...
    public ConsumerFactory<String, String> commandReplyConsumerFactory() {
        Map<String, Object> configProps = new HashMap<>();
        configProps.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, bootstrapServers);
        configProps.put(ConsumerConfig.GROUP_ID_CONFIG, "${kafka.command.reply.group-id}");
        configProps.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
        configProps.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
        ...
        log.info("✅ Configurando CommandReplyConsumer con group-id: ${kafka.command.reply.group-id}");
        return new DefaultKafkaConsumerFactory<>(configProps);
    }

    /**
     * Container Factory para respuestas de comandos
     */
    @Bean
    public ConcurrentKafkaListenerContainerFactory<String, String> commandReplyKafkaListenerContainerFactory(
        ...
        factory.setConcurrency(2); // Baja concurrencia (respuestas son rápidas)
        factory.getContainerProperties().setAckMode(ContainerProperties.AckMode.MANUAL);
